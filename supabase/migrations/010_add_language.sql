-- Migration 010: Add language support for Japanese products
-- Run this in Supabase SQL Editor

-- Step 1: Add language column to sets and products
ALTER TABLE sets ADD COLUMN IF NOT EXISTS language TEXT DEFAULT 'en';
ALTER TABLE products ADD COLUMN IF NOT EXISTS language TEXT DEFAULT 'en';

-- Step 2: Composite unique on sets (code + language) to avoid collisions
ALTER TABLE sets DROP CONSTRAINT IF EXISTS sets_code_key;
CREATE UNIQUE INDEX IF NOT EXISTS sets_code_language_idx ON sets (code, language);

-- Step 3: Drop and recreate materialized view with language columns
