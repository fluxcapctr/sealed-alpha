...
- Value (master set value / booster box price ratio — auto-computed from DB)
- Set Depth (chase diversity beyond the #1 card)

Overall grade is auto-computed from weighted sub-scores.
Value dimension is auto-computed from DB data (total_set_value / booster box price).
Sets without booster box data use a manual fallback stored in the "value" key.
...
def ratio_to_value_score(ratio: float) -> int:
    """Convert master-set-value / booster-box-price ratio to a 1-10 score."""
    if ratio >= 5.0: return 10
    if ratio >= 3.5: return 9
...
def compute_value_scores(db) -> dict[str, int]:
    """Fetch total_set_value and latest booster box prices, return {set_name: value_score}."""
    # Get EN sets with total_set_value
    sets_data = db.client.table("sets").select(
        "id, name, total_set_value"
    ).eq("language", "en").not_.is_("total_set_value", "null").execute().data

    sets_by_id = {s["id"]: s for s in sets_data}

    # Get booster box products
    products = db.client.table("products").select(
        "id, set_id, name"
    ).like("name", "%Booster Box%").execute().data

    # Get latest price for each booster box
    box_prices: dict[str, float] = {}
    for p in products:
        sid = p["set_id"]
        snaps = db.client.table("price_snapshots").select(
            "market_price"
        ).eq("product_id", p["id"]).not_.is_(
            "market_price", "null"
        ).order("snapshot_date", desc=True).limit(1).execute().data

        if snaps and snaps[0]["market_price"]:
            price = snaps[0]["market_price"]
            if sid not in box_prices or price < box_prices[sid]:
                box_prices[sid] = price

    # Compute ratios and scores
    result: dict[str, int] = {}
    for sid, s in sets_by_id.items():
        if sid in box_prices and box_prices[sid] > 0:
            ratio = s["total_set_value"] / box_prices[sid]
            result[s["name"].lower()] = ratio_to_value_score(ratio)
...
# chase/art/nostalgia/fun/depth = 1-10 scores (subjective)
# value = manual fallback (1-10) used only when DB has no booster box price data
...
SET_SCORES = {
    ...
    "Scarlet & Violet": {
        "grade": "B",
        "chase": 6, "art": 7, "nostalgia": 4, "fun": 7, "value": 5, "depth": 6,
    ...
    "Paldea Evolved": {
        "grade": "A",
        "chase": 9, "art": 8, "nostalgia": 5, "fun": 7, "value": 7, "depth": 9,
        "chase_card": "Iono SIR",
    ...
    "Obsidian Flames": {
        "grade": "D",
        "chase": 7, "art": 5, "nostalgia": 4, "fun": 5, "value": 4, "depth": 3,
    ...
    "Paradox Rift": {
        "grade": "B",
        "chase": 6, "art": 7, "nostalgia": 4, "fun": 7, "value": 5, "depth": 7,
    ...
    "151": {
        "grade": "S",
        "chase": 10, "art": 9, "nostalgia": 10, "fun": 8, "value": 8, "depth": 9,
        "chase_card": "Charizard ex SIR",
    ...
    "Paldean Fates": {
        "grade": "B",
        "chase": 8, "art": 7, "nostalgia": 6, "fun": 7, "value": 5, "depth": 7,
    ...
    "Twilight Masquerade": {
        "grade": "B",
        "chase": 7, "art": 8, "nostalgia": 4, "fun": 7, "value": 5, "depth": 6,
    ...
    "Stellar Crown": {
        "grade": "C",
        "chase": 5, "art": 6, "nostalgia": 6, "fun": 6, "value": 5, "depth": 4,
    ...
    "Surging Sparks": {
        "grade": "A",
        "chase": 9, "art": 8, "nostalgia": 7, "fun": 7, "value": 6, "depth": 7,
    ...
    "Prismatic Evolutions": {
        "grade": "A",
        "chase": 10, "art": 9, "nostalgia": 8, "fun": 7, "value": 7, "depth": 9,
    ...
    "Journey Together": {
        "grade": "A",
        "chase": 7, "art": 8, "nostalgia": 6, "fun": 7, "value": 5, "depth": 7,
    ...
    "Sword & Shield": {
        "grade": "C",
        "chase": 6, "art": 5, "nostalgia": 3, "fun": 6, "value": 5, "depth": 4,
    ...
    "Darkness Ablaze": {
        "grade": "C",
        "chase": 8, "art": 5, "nostalgia": 4, "fun": 5, "value": 5, "depth": 3,
    ...
    "Chilling Reign": {
        "grade": "B",
        "chase": 7, "art": 8, "nostalgia": 6, "fun": 6, "value": 6, "depth": 7,
    ...
    "Evolving Skies": {
        "grade": "S",
        "chase": 10, "art": 9, "nostalgia": 8, "fun": 6, "value": 9, "depth": 10,
    ...
    "Astral Radiance": {
        "grade": "B",
        "chase": 7, "art": 7, "nostalgia": 6, "fun": 7, "value": 6, "depth": 7,
    ...
    "Lost Origin": {
        "grade": "B",
        "chase": 8, "art": 8, "nostalgia": 7, "fun": 6, "value": 6, "depth": 6,
    ...
    "Silver Tempest": {
        "grade": "B",
        "chase": 7, "art": 7, "nostalgia": 6, "fun": 6, "value": 6, "depth": 6,
    ...
    "Pokémon GO": {
        "grade": "C",
        "chase": 6, "art": 5, "nostalgia": 5, "fun": 6, "value": 5, "depth": 4,
    ...
    "Team Up": {
        "grade": "S",
        "chase": 9, "art": 8, "nostalgia": 9, "fun": 7, "value": 9, "depth": 8,
    ...
    "Unbroken Bonds": {
        "grade": "A",
        "chase": 8, "art": 8, "nostalgia": 7, "fun": 7, "value": 8, "depth": 8,
    ...
    "Unified Minds": {
        "grade": "B",
        "chase": 8, "art": 7, "nostalgia": 7, "fun": 6, "value": 7, "depth": 6,
    ...
    "Cosmic Eclipse": {
        "grade": "A",
        "chase": 8, "art": 9, "nostalgia": 7, "fun": 7, "value": 8, "depth": 9,
    ...
    "Hidden Fates": {
        "grade": "A",
        "chase": 9, "art": 8, "nostalgia": 7, "fun": 8, "value": 9, "depth": 8,
    ...
    "Shining Legends": {
        "grade": "B",
        "chase": 7, "art": 7, "nostalgia": 7, "fun": 7, "value": 7, "depth": 6,
    ...
    "Phantom Forces": {
        "grade": "B",
        "chase": 7, "art": 6, "nostalgia": 7, "fun": 6, "value": 8, "depth": 6,
    ...
    "Roaring Skies": {
        "grade": "B",
        "chase": 8, "art": 7, "nostalgia": 7, "fun": 6, "value": 8, "depth": 6,
    ...
    "Fates Collide": {
        "grade": "C",
        "chase": 6, "art": 5, "nostalgia": 5, "fun": 5, "value": 7, "depth": 4,
...
async def main():
    ...
    # Auto-compute value scores from DB (total_set_value / booster box price)
    logger.info("Computing value scores from DB (set value / booster box price)...")
    db_value_scores = compute_value_scores(db)
    logger.info(f"Auto-computed value scores for {len(db_value_scores)} sets")
    ...
    for set_name, scores in SET_SCORES.items():
        ...
        # Use auto-computed value score if available, otherwise manual fallback
        effective_value = db_value_scores.get(key, scores["value"])
        scores_with_value = {**scores, "value": effective_value}

        grade, avg = compute_grade(scores_with_value)
    ...
    if args.dry_run:
        ...
        for name, row, avg in rows_to_upsert:
            logger.info(
                ...
                f"Val:{row['value_score']} Dep:{row['set_depth_score']} | "
